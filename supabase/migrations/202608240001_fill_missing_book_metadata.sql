...
create or replace function public.fill_missing_book_metadata(
  p_book_id bigint,
  p_source text,
  p_external_id text,
  p_isbn text,
...
begin
  ...
  if existing_book.external_id is not null
    and existing_book.external_id <> coalesce(p_external_id, '')
  then
  ...
  if p_publication_year is not null
    and (p_publication_year < 1 or p_publication_year > 2200)
  then
