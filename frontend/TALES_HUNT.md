...
`src/lib/talesHunt.js` owns the flag `TALES_HUNT_ENABLED`, count, reducer, and storage key `litshelf-word-hunt-v2`. The former event key is left untouched and is not loaded. Refresh preserves this round’s letters and arrangement; blocked storage falls back to session state. Reset is available in the development tracker.

Existing collection flight, bookmark styling, and varied placements remain. Seven puzzle tiles scroll horizontally when needed to preserve 44px touch targets. New files were not required for this update; changes span the shared hunt components/state/CSS, notification inbox, discovery locations, i18n, and hunt tests.
